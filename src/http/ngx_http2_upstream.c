...
typedef struct ngx_http2_upstream_server_s{
...
static void ngx_http2_upstream_connect(ngx_http_request_t *r) {
	...
	if (rc == NGX_BUSY) {
		...
		ngx_http2_upstream_next(r, u, ngx_http2_upstream_FT_NOLIVE);
		return;
	}
	...
	u->write_event_handler = ngx_http2_upstream_send_request_handler;
	u->read_event_handler = ngx_http2_upstream_process_header;
...
static void ngx_http2_upstream_default_choose_server(ngx_http_request_t* request,ngx_http2_upstream_srv_conf_t *scf){
	...
	for(i=0;i < scf->servers.nelts;++i){
		if((!ngx_queue_empty(&server[i].connection_queue)) || (server[i].use_conns < server[i].max_conns)){
