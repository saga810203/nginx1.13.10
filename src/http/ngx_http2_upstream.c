...
static void ngx_http2_upstream_free_stream_with_request(void *data){
	//TODO:  free with  client error ;
	ngx_http2_stream_t *stream;
	stream = data;
	if(stream->state != NGX_HTTP2_STREAM_STATE__CLOSED && (!stream->event.error)){
		//rst_stream
...
ngx_int_t ngx_http2_upstream_create(ngx_http_request_t *r) {
	...
	cln->data = u;
	cln->handler =ngx_http2_upstream_free_stream_with_request;
...
ngx_http2_upstream_srv_conf_t* ngx_http2_upstream_get_srv_conf(ngx_conf_t *cf, u_char* name, size_t name_len) {
	...
	uscfp = umcf->upstreams.elts;
	for (i = 0; i < umcf->upstreams.nelts; i++) {
		if (uscfp[i]->host.len == name_len && ngx_strncasecmp(uscfp[i]->host.data, name, name_len) == 0) {
			return uscfp[i];
		}
	}
	return NULL;
