...
static void ngx_resolver_process_a(ngx_resolver_t *r, u_char *buf, size_t n,
		ngx_uint_t ident, ngx_uint_t code, ngx_uint_t qtype, ngx_uint_t nan,
		ngx_uint_t trunc, ngx_uint_t ans);
static void ngx_resolver_process_srv(ngx_resolver_t *r, u_char *buf, size_t n,
		ngx_uint_t ident, ngx_uint_t code, ngx_uint_t nan, ngx_uint_t trunc,
		ngx_uint_t ans);
...
ngx_resolver_create(ngx_conf_t *cf, ngx_str_t *names, ngx_uint_t n) {
	...
	if (n) {
		if (ngx_array_init(&r->connections, cf->pool, n,
				sizeof(ngx_resolver_connection_t)) != NGX_OK) {
			return NULL;
		}
	}
...
ngx_int_t ngx_resolve_name(ngx_resolver_ctx_t *ctx) {
	...
	if (ctx->service.len) {
		...
		if (ngx_strlchr(ctx->service.data, ctx->service.data + ctx->service.len,
				'.') == NULL) {
...
static void ngx_resolver_process_response(ngx_resolver_t *r, u_char *buf,
		size_t n, ngx_uint_t tcp) {
	char *err;
	ngx_uint_t i, times, ident, qident, flags, code, nqs, nan, trunc, qtype,
			qclass;
#if (NGX_HAVE_INET6)
	ngx_uint_t qident6;
#endif
	...
	if ((flags & 0xf870) != 0x8000 || (trunc && tcp)) {
		ngx_log_error(r->log_level, r->log, 0,
				"invalid %s DNS response %ui fl:%04Xi", tcp ? "TCP" : "UDP",
				ident, flags);
		return;
	}
	...
	ngx_log_error(r->log_level, r->log, 0,
			"DNS error (%ui: %s), query id:%ui, name:\"%*s\"", code,
...
static void ngx_resolver_process_a(ngx_resolver_t *r, u_char *buf, size_t n,
	...
	if (ngx_resolver_copy(r, &name, buf, buf + sizeof(ngx_resolver_hdr_t),
			buf + n) != NGX_OK) {
		return;
	}
	...
	if (rn == NULL) {
		ngx_log_error(r->log_level, r->log, 0, "unexpected response for %V",
				&name);
		ngx_resolver_free(r, name.data);
		goto failed;
	}
	...
	if (ident != qident) {
		ngx_log_error(r->log_level, r->log, 0,
				"wrong ident %ui response for %V, expect %ui", ident, &name,
				qident);
		ngx_resolver_free(r, name.data);
		goto failed;
	}
	...
	for (a = 0; a < nan; a++) {
		...
		ttl = (an->ttl[0] << 24) + (an->ttl[1] << 16) + (an->ttl[2] << 8)
				+ (an->ttl[3]);

		if (class != 1) {
			ngx_log_error(r->log_level, r->log, 0, "unexpected RR class %ui",
					class);
			goto failed;
		}
		...
		default:
			...
			ngx_log_error(r->log_level, r->log, 0, "unexpected RR type %ui",
					type);
		}
...
static void ngx_resolver_rbtree_insert_addr6_value(ngx_rbtree_node_t *temp,
	...
	for (;;) {
		...
		if (node->key < temp->key) {
			...
			p = (ngx_memcmp(&rn->addr6, &rn_temp->addr6, 16) < 0) ?
...
#if (NGX_HAVE_INET6)
	case AF_INET6:
		len = sizeof(ngx_resolver_hdr_t) + 64 + sizeof(".ip6.arpa.") - 1
